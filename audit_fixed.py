...
for fname in files:
    try:
        ...
        # Check for FAQ section (general check)
        has_faq = bool(re.search(r'class=["\'].*?faq', html, re.I))
        
        # Count FAQ items - handle various patterns:
        # Pattern 1: msv__faq-item class (for pillar pages)
        faq_items = len(re.findall(r'class=["\'][^"\']*faq[_-]item', html, re.I))
        
        # Pattern 2: If not found, look for <details> tags with faq in class
        if faq_items == 0:
            faq_items = len(re.findall(r'<details[^>]*class=["\'][^"\']*faq', html, re.I))
        
        # Pattern 3: If not found, count all <details> tags (for files using details/summary)
        if faq_items == 0:
            faq_items = len(re.findall(r'<details\s*(?:>|[^>]*>)', html, re.I))
