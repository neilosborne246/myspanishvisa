...
# Process posts 101-150 from the list
with open('/tmp/posts_needing_faqs.txt', 'r') as f:
    all_posts = [line.strip() for line in f.readlines()]
    posts_to_process = all_posts[100:150]

print(f"Processing {len(posts_to_process)} posts in Batch 3...")
success_count = 0
...
print(f"\n✓ Batch 3 Complete: {success_count}/{len(posts_to_process)} posts successfully expanded to 12 FAQs")
print(f"\nRemaining posts for Batches 4-5: {232 - 150}")
