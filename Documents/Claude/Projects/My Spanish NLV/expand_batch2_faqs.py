...
# Process posts 51-100 from the list
with open('/tmp/posts_needing_faqs.txt', 'r') as f:
    all_posts = [line.strip() for line in f.readlines()]
    posts_to_process = all_posts[50:100]

print(f"Processing {len(posts_to_process)} posts in Batch 2...")
success_count = 0
...
print(f"\n✓ Batch 2 Complete: {success_count}/{len(posts_to_process)} posts successfully expanded to 12 FAQs")
print(f"\nRemaining posts for Batches 3-5: {232 - 100}")
