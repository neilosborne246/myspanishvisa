...
# Process posts 201-232 from the list
with open('/tmp/posts_needing_faqs.txt', 'r') as f:
    all_posts = [line.strip() for line in f.readlines()]
    posts_to_process = all_posts[200:232]

print(f"Processing {len(posts_to_process)} posts in Batch 5...")
success_count = 0
...
print(f"\n✓ Batch 5 Complete: {success_count}/{len(posts_to_process)} posts successfully expanded to 12 FAQs")
print(f"\n✅ ALL BATCHES COMPLETE: 232/232 posts have been expanded from 6 to 12 FAQs each!")
