...
for fname in files:
    try:
        ...
        # Count FAQ items - try multiple patterns
        faq_items = len(re.findall(r'class=["\'].*?faq-item', html, re.I))
        if faq_items == 0:
            # Try h3 within faq sections or similar
            faq_items = len(re.findall(r'<button[^>]*class=["\'][^"\']*faq', html, re.I))
        if faq_items == 0:
            # Try counting <h3> tags within FAQ
            faq_match = re.search(r'class=["\'][^"\']*faq[^"\']*["\'].*?</section>', html, re.I | re.DOTALL)
            if faq_match:
                faq_items = len(re.findall(r'<h3|<h4', faq_match.group(0), re.I))
