...
for fname in files:
    try:
        ...
        # Count FAQ items - three patterns:
        # 1. msv__faq-item class
        # 2. <details class="msv__faq-list"> structure
        # 3. Generic <details> tags (fallback)
        faq_items = len(re.findall(r'class=["\'][^"\']*faq[_-]item', html, re.I))
        if faq_items == 0:
            # Try counting <details class="msv__faq-list">
            faq_items = len(re.findall(r'<details[^>]*class=["\'][^"\']*faq', html, re.I))
        if faq_items == 0:
            # Try counting generic <details> tags as last resort
            faq_items = len(re.findall(r'<details>', html, re.I))
